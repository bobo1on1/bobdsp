...
int  SetupSignals();
void ProcessMessages(bool& stop, bool& portregistered, bool& portconnected, bool usetimeout,
                     vector< pair<int, CJackClient*> > pipes, int& signalsfd);
void ProcessSignalfd(int& signalsfd, bool& stop);
void LoadLadspaPaths(vector<string>& ladspapaths);
bool LoadPluginsFromFile(vector<CLadspaPlugin*> plugins, vector<CJackClient*>& clients);
void LoadPluginsFromRoot(TiXmlElement* root, vector<CLadspaPlugin*> plugins, vector<CJackClient*>& clients);
bool LoadPortsFromPlugin(TiXmlElement* plugin, vector<portvalue>& portvalues);
CLadspaPlugin* SearchLadspaPlugin(vector<CLadspaPlugin*> plugins, int64_t uniqueid, const char* label);
bool PortDescriptorSanityCheck(CLadspaPlugin* plugin, unsigned long port, LADSPA_PortDescriptor p);
bool LoadConnectionsFromFile(CPortConnector& portconnector);
...
int main (int argc, char *argv[])
{
  ...
  //load ladspa plugins
  vector<CLadspaPlugin*> plugins;
  for (vector<string>::iterator it = ladspapaths.begin(); it != ladspapaths.end(); it++)
    CLadspaPlugin::GetPlugins(*it, plugins);

  Log("Found %zu plugins", plugins.size());
  ...
  vector<CJackClient*> clients;
  LoadPluginsFromFile(plugins, clients);

  //load the port connections
  CPortConnector portconnector;
  LoadConnectionsFromFile(portconnector);

  //set up signal handlers
  int signalsfd = SetupSignals();

  Log("Starting %zu jack client(s)", clients.size());
  ...
  while(!stop)
  {
    ...
    for (vector<CJackClient*>::iterator it = clients.begin(); it != clients.end(); it++)
    {
    ...
    ProcessMessages(stop, portregistered, portconnected,
                    !allconnected || portregistered || portconnected, pipes, signalsfd);

    LogDebug("main loop woke up");
  }

  Log("Stopping %zu jack client(s)", clients.size());
  for (vector<CJackClient*>::iterator it = clients.begin(); it != clients.end(); it++)
    delete *it;
...
int SetupSignals()
{
  ...
  int signalsfd = signalfd(-1, &sigset, SFD_NONBLOCK);
  if (signalsfd == -1)
  {
                     ...
                     vector< pair<int, CJackClient*> > pipes, int& signalsfd)
{
  ...
  else if (returnv > 0)
  {
    ...
    //check for signals
    ProcessSignalfd(signalsfd, stop);
  }
...
void ProcessSignalfd(int& signalsfd, bool& stop)
{
  ...
  int returnv = read(signalsfd, &siginfo, sizeof(siginfo));
...
bool LoadPluginsFromFile(vector<CLadspaPlugin*> plugins, vector<CJackClient*>& clients)
{
  ...
  LoadPluginsFromRoot(root, plugins, clients);
...
void LoadPluginsFromRoot(TiXmlElement* root, vector<CLadspaPlugin*> plugins, vector<CJackClient*>& clients)
{
  ...
  for (TiXmlElement* plugin = root->FirstChildElement("plugin"); plugin != NULL; plugin = plugin->NextSiblingElement("plugin"))
  {
    ...
    CLadspaPlugin* ladspaplugin = SearchLadspaPlugin(plugins, uniqueid_p, label->GetText());
    if (ladspaplugin)
    {
...
bool LoadConnectionsFromFile(CPortConnector& portconnector)
{
       ...
       xmlconnection = xmlconnection->NextSiblingElement("connection"))
  {
    ...
    portconnector.AddConnection(connection);
  }
